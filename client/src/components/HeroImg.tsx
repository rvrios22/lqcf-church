...
function HeroImg({ name, text }: HeroImgTypes) {
  ...
  return (
    <div className="relative m-auto mt-6 mb-6 flex w-4/5 justify-center">
      <Image
        srcSet={`
  ...
  `}
    ...
    80vw
  "
        className="m-auto h-[150px] object-cover lg:h-[50vh] lg:object-fill"
        src={`api/static/imgs/${name}/${name}-1000.webp`}
        alt={`${name} header`}
        onLoad={() => setLoaded(true)}
      />
      <h1
        className={`absolute left-1/2 z-11 -translate-x-1/2 -translate-y-1/2 text-center font-[Inter,'Tahoma','Verdana',sans-serif] text-white transition-all duration-1000 ease-in-out [text-shadow:3px_2px_3px_rgb(0,0,0)] md:text-2xl lg:text-5xl ${loaded ? "top-1/2 opacity-100" : "top-[60%] opacity-0"} `}
      >
        {text}
      </h1>
    </div>
